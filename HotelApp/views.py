...
def Home(request):
    if request.method == 'POST':
        ...
        # check_in_stripped = check_in.split(" ")[0]
        # print(check_in_stripped)

        # check_out_stripped = check_out.split(" ")[0]
        # print(check_out_stripped)

        if check_in and check_out and adults and children:
            check_in = datetime.strptime(check_in, '%Y/%m/%d %H:%M').date()
            check_out = datetime.strptime(check_out, '%Y/%m/%d %H:%M').date()
            adults = int(adults)
            children = int(children)


            # Perform availability check logic
            available_rooms = Add_Room.objects.filter(
                    ~Q(online_booking__Check_out__gt=check_in)\
                          & ~Q(online_booking__Check_in__lt=check_out)
            )
            ...
            # return redirect('available', Check_in=check_in, 
            #                 Check_out=check_out, 
            #                 adults=adults, 
            #                 children=children)
...
def room_booking(request, room_id):
    ...
    if occupied:
        return render(request, 'occupied.html')
    
    if request.method == 'POST':
        # print(f'the items from the requests are: {request.POST.items}')
        # Process the booking form data
        # Retrieve the selected room using the room_id

        # room = Add_Room.objects.get(Id=room_id)
        # Access the form data using request.POST dictionary
        check_in = request.POST.get('check_in')
        check_out = request.POST.get('check_out')
        ...
        # Save the booking details to the Online_Booking model
        booking = models.Online_Booking(
            Check_in=check_in,
            Check_out=check_out,
            ADULT=adult,
            CHILDREN=children,
            ...
            Img=img,
            Address=address,
            room=room  # Set the room for the booking
        )
        booking.save()
...
def check_availability(request):
    if request.method == 'POST':
        ...
        # Perform availability check logic
        available_rooms = Add_Room.objects.exclude(
            online_booking__Check_out__gte=check_in,
            online_booking__Check_in__lte=check_out
        )
...
def Add_room(request):
    if request.method == 'POST':
        ...
        if request.method == 'POST':
            ...
            Data.Room_Description = request.POST.get('Room_Description')
